...
class ColumnLineageMixin:
    def get_column_lineage(
            self, exclude_path_ending_in_subquery=True, exclude_subquery_columns=True
    ) -> Set[Tuple[Column, ...]]:
        """s
        ...
        for source, target in itertools.product(source_columns, target_columns):
            ...
            for path in simple_paths:
                if exclude_subquery_columns:
                    print("exclude_subquery_columns", exclude_subquery_columns)
                    path = [
                    ...
                    for node in path:
                        ...
                        print("candiates:", node.parent_candidates)
                    if len(path) > 1:
